...
## 台詞を伸ばしたときに起きたこと（結びの差し替えで実測）

- **cap は素材を切り詰めるたびに古くなる。** narration.json の cap（finale 9.89）に対して実素材は 5.79 秒だった。layout.py は cap しか見ないので判定は「OK」、build-flat で初めて「素材が足りません」で落ちる。cap は `ffprobe` の実尺から更新するか、layout 側で実尺を読むこと。
